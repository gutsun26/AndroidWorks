...
public class CustomerRecordsDeleteActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener
{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState)
    {
        ...
        //numberToBeDeleted       =       (TextView)      findViewById(R.id.numberToBeDeleted);
    ...
    public void onItemSelected (AdapterView<?> parent, View view, int position, long id)
    {
        ...
        if(choiceMade.equalsIgnoreCase(clientLiteral))
        {
            ...
            {
                record.setVisibility(View.VISIBLE);
                record.setText("DELETED ALL RECORDS OF :"+clientChosen);
                //deleteMessage();
            }
        }
    ...
    public void deleteMessage()
    {
        ...
        Cursor c = getContentResolver().query(Uri.parse("content://sms/inbox"), columns, null, null, null);
        try
        {
            if(c.moveToFirst())
            {
                do
                {
                    try
                    {
                        String adds = c.getString(2);
                        String MobileNumber = "+91"+numberChosenToDelete;
                        MobileNumber = MobileNumber.trim();
                        //address.setText(c.getString(1)+"---"+c.getString(2));
                        numberToBeDeleted.setText(MobileNumber);
                        //toast(c.getString(2));
                        if (adds.trim().equalsIgnoreCase(MobileNumber))
                        {
                            address.setText(adds);
                            //toast("INSIDE");
                            String pid = c.getString(1);
                            String uri = "content://sms/inbox/";
                            rows = getContentResolver().delete(Uri.parse(uri), null, null);
                            toast(rows+ " deleted");
                            record.setText("DELETED FROM INBOX--"+rows);
                            break;
                        }
                        else
                        {
                            record.setText("NO MATCH--"+rows);
                        }
                    }
                    catch (Exception e)
                    {
                       record.setText(e.toString());
                    }
                }while (c.moveToNext());
                c.close();
            }
        }catch(NullPointerException ne){toast(ne.getMessage());}
    }
    //---deletes a particular title---
    public boolean deleteDateRecord(String name)
    {
        customerSMSReceiverControllerDB = new CustomerSMSReceiverControllerDB(this);
        theSqLiteDatabase= customerSMSReceiverControllerDB.getReadableDatabase();
        return theSqLiteDatabase.delete("G_CUSTOMER_SMS", "DATE" + "=" + name, null) > 0;
    }

    public boolean deleteClientRecord(String name)
    {
        return theSqLiteDatabase.delete("G_CUSTOMER_SMS", "NUMBER" + "=" + name, null) > 0;
    }
