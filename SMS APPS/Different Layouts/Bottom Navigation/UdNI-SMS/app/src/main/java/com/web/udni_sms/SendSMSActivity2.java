...
public class SendSMSActivity2 extends AppCompatActivity //implements AdapterView.OnItemSelectedListener //ExpandableListActivity //FragmentActivity
{
    /******************************MODEL****************************************/
    /*SMS SENDING RELATED*/
            /*
    ...
    */
    ...
    com.web.udni_sms.builder_db.BuilderSMSReceiverControllerDB builderSMSReceiverControllerDB;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState)
    {
        ...
        builderSMSReceiverControllerDB = new BuilderSMSReceiverControllerDB(this);
        theSqLiteDatabase= builderSMSReceiverControllerDB.getReadableDatabase();
        theQuery = "CREATE TABLE IF NOT EXISTS BUILDER_SMS(Id INTEGER PRIMARY KEY AUTOINCREMENT, DATE TEXT, ADDRESS TEXT, CONTENT TEXT);";
        ...
        query = "SELECT BUILDER_CLIENT_NAME FROM BUILDER_CLIENT_DATA WHERE BUILDER_CLIENT_NAME='"+theKey+"'";
        ...
        if(rowCount!=0)
        {
            if (cursor.moveToFirst())
            {
                do
                {
                    adminText.setText(cursor.getString(cursor.getColumnIndex("BUILDER_CLIENT_NAME")));
        ...
        builderControllerDB = new BuilderControllerDB(this);
        sqLiteDatabase = builderControllerDB.getReadableDatabase();
        contactNumberQuery = "SELECT BUILDER_CLIENT_NUMBER FROM BUILDER_CLIENT_DATA WHERE BUILDER_CLIENT_NAME='"+theKey+"'";
        ...
        if(rowCount2!=0)
        {
            if (cursor2.moveToFirst())
            {
                do
                {
                    choice = cursor2.getString(cursor2.getColumnIndex("BUILDER_CLIENT_NUMBER"));
        ...
        theSqLiteDatabase= builderSMSReceiverControllerDB.getReadableDatabase();
        //clientCursor = theSqLiteDatabase.rawQuery("SELECT CONTENT FROM BUILDER_SMS WHERE DATE='"+date+"'",null);
        clientCursor = theSqLiteDatabase.rawQuery("SELECT CONTENT FROM BUILDER_SMS WHERE ADDRESS='"+choice+"'",null);
    ...
    */
    ...
    BroadcastReceiver smsReceiver = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (intentExtras != null)
            {
                ...
                {
                    SendSMSActivity2.this.getReceivedSMS(theString);
                }
            }
        }//end of onReceiver()
    ...
    public void getReceivedSMS(final String smsMessage)
    {
        ...
        if (cursor.moveToFirst())
        {
            do
            {
                THENUMBER.add(cursor.getString(cursor.getColumnIndex("BUILDER_CLIENT_NUMBER")));
            ...
            if(!THENUMBER.isEmpty())
            {
                for(int i=0; i<THENUMBER.size(); i++)
                {
                    if(THENUMBER.get(i).equalsIgnoreCase(cellNumber))
                    {
                        ...
                        theQuery = "CREATE TABLE IF NOT EXISTS BUILDER_SMS(Id INTEGER PRIMARY KEY AUTOINCREMENT, DATE TEXT, ADDRESS TEXT, CONTENT TEXT);";
                        theSqLiteDatabase.execSQL(theQuery);
                        theSqLiteDatabase.execSQL("INSERT INTO BUILDER_SMS(DATE, ADDRESS, CONTENT) VALUES('"+date+"','"+cellNumber+"','"+extractedMessage+"')");
                ...
                for(int i=0; i<THENUMBER.size(); i++)
                {
                    ...
                    {
                        //clientCursor = theSqLiteDatabase.rawQuery("SELECT CONTENT FROM BUILDER_SMS WHERE DATE='"+date+"'",null);
                        clientCursor = theSqLiteDatabase.rawQuery("SELECT CONTENT FROM BUILDER_SMS WHERE ADDRESS='"+cellNumber+"'",null);
                        theContent = new ArrayList<String>();
                        theContent.clear();
                        if (clientCursor.moveToFirst())
                        {
    ...
    {
        theSqLiteDatabase= builderSMSReceiverControllerDB.getReadableDatabase();
        theSqLiteDatabase.execSQL("DROP TABLE BUILDER_SMS");
        toast("THE TABLE BUILDER_SMS IS DELETED");
    }
    @Override
